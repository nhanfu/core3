...
export class HybridDuckDbDatabase {
  ...
  async partition(definition: PartitionDefinition): Promise<void> {
    ...
    if (['range', 'time', 'year'].includes(definition.strategy) && !interval) throw new Error('Range/time partitions require an interval');
    if (['range', 'time', 'year'].includes(definition.strategy) && interval !== 'year') throw new Error(`Range interval is not implemented yet: ${interval}`);
