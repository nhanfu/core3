  ...
  value.forEach((source, index) => {
    ...
    if (source.workflow !== undefined) {
      ...
      if (isRecord(source.workflow)) {
        rejectUnknownKeys(source.workflow, new Set(['handler', 'permission', 'status_source', 'state_source', 'status_storage', 'allow_add', 'transitions', 'actions']), `${path}.workflow`, issues);
        ...
        if (source.workflow.status_storage !== undefined) requireRecord(source.workflow.status_storage, `${path}.workflow.status_storage`, issues);
