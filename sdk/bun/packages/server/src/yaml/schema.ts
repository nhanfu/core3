...
const FIELD_KEYS = new Set(['field', 'label', 'type', 'required', 'readonly', 'options', 'options_source', 'multiple', 'default', 'tokens', 'show_if', 'placeholder', 'search_placeholder', 'currency', 'decimals', 'min', 'max', 'step', 'wide', 'palette']);
...
const ACTION_KEYS: Record<ActionDefinition['type'], Set<string>> = {
  ...
  server_form: new Set(['id', 'type', 'title', 'description', 'action', 'prefill', 'prefill_source', 'refresh', 'fields', 'params', 'permission', 'permissions', 'handler', 'service', 'service_operation', 'workflow', 'operation', 'domain', 'kind', 'topic', 'topic_version', 'event', 'mutation', 'success_message', 'modal_style', 'submit_label', 'cancel_label']),
...
const COMPONENT_KEYS = new Map<string, Set<string>>([
  ...
  ['OdooFormView', new Set(['type', 'source', 'title_field', 'subtitle_field', 'avatar_field', 'avatar_initials_field', 'status_field', 'status_label_field', 'status_colors', 'statusbar', 'statusbar_actions', 'statusbar_source', 'status_badges', 'form_mode', 'editable', 'initial_editing', 'fields', 'edit_fields', 'relations', 'groups', 'group_columns', 'notebook', 'header_actions', 'action_menu', 'stat_buttons', 'labels', 'message_source', 'message_page_size', 'follower_source', 'follower_candidates_source', 'follower_candidates_page_size', 'attachment_source', 'attachment_page_size', 'attachment_panel_open', 'chatter_label', 'chatter_empty', 'message_label', 'note_label', 'activity_label', 'message_action', 'note_action', 'activity_action', 'message_placeholder', 'note_placeholder', 'activity_placeholder', 'send_label', 'log_label', 'schedule_label', 'cancel_label', 'follower_label', 'attachment_label', 'add_follower_label', 'remove_follower_label', 'no_followers_label', 'follower_search_placeholder', 'add_label', 'add_attachment_label', 'no_attachments_label', 'uploading_label', 'upload_failed_label', 'download_label', 'preview_label', 'attachment_accept', 'follower_add_action', 'follower_remove_action', 'attachment_upload_action', 'attachment_download_action', 'attachment_actions', 'message_actor_field', 'message_action_field', 'message_detail_field', 'message_detail_labels', 'message_timestamp_field', 'message_action_labels', 'save_label', 'discard_label', 'edit_details_label', 'workflow_status_label', 'content_slot'])],
  ...
  ['SpreadsheetDashboardClientAction', new Set(['type', 'id', 'groups_source', 'dashboards_source', 'workbooks_source', 'summaries_source', 'chart_source', 'rows_source', 'default_dashboard_id', 'read_only', 'favorite_action'])],
...
function validateComponents(
  ...
  value.forEach((component, index) => {
    ...
    if (component.type === 'OdooFormView') {
      ...
      if (component.action_menu !== undefined) {
        requireRecord(component.action_menu, `${path}.action_menu`, issues);
        if (isRecord(component.action_menu)) {
          rejectUnknownKeys(component.action_menu, FORM_ACTION_MENU_KEYS, `${path}.action_menu`, issues);
          if (component.action_menu.label !== undefined) requireString(component.action_menu.label, `${path}.action_menu.label`, issues);
          if (component.action_menu.icon !== undefined) requireString(component.action_menu.icon, `${path}.action_menu.icon`, issues);
          if (component.action_menu.aria_label !== undefined) requireString(component.action_menu.aria_label, `${path}.action_menu.aria_label`, issues);
          if (!Array.isArray(component.action_menu.actions)) {
            issues.push(`${path}.action_menu.actions must be an array`);
          } else {
            for (const [index, item] of component.action_menu.actions.entries()) {
              const itemPath = `${path}.action_menu.actions[${index}]`;
              requireRecord(item, itemPath, issues);
              if (!isRecord(item)) continue;
              rejectUnknownKeys(item, COMPONENT_ACTION_KEYS, itemPath, issues);
              requireString(item.id, `${itemPath}.id`, issues);
              if (typeof item.id === 'string' && !actionIds.has(item.id)) {
                issues.push(`${itemPath}.id references unknown action "${item.id}"`);
              }
            }
          }
        }
      }
      if (component.stat_buttons !== undefined) {
        if (!Array.isArray(component.stat_buttons)) issues.push(`${path}.stat_buttons must be an array`);
        const statButtons = Array.isArray(component.stat_buttons) ? component.stat_buttons : [];
        for (const [index, button] of statButtons.entries()) {
          requireRecord(button, `${path}.stat_buttons[${index}]`, issues);
          if (isRecord(button)) rejectUnknownKeys(button, FORM_STAT_KEYS, `${path}.stat_buttons[${index}]`, issues);
          requireString(button.id, `${path}.stat_buttons[${index}].id`, issues);
          requireString(button.value_field, `${path}.stat_buttons[${index}].value_field`, issues);
          if (typeof button.id === 'string' && !actionIds.has(button.id)) {
            issues.push(`${path}.stat_buttons[${index}].id references unknown action "${button.id}"`);
          }
        }
      }
    }
...
function validateColumns(value: unknown, path: string, actionIds: Set<string>, issues: string[]) {
  ...
  value.forEach((column, index) => {
    ...
    if (column.width !== undefined && (!Number.isFinite(column.width) || column.width <= 0)) {
      issues.push(`${columnPath}.width must be a positive number`);
    }
