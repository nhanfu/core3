...
export class FilterBar extends BaseComponent {
  ...
  draw(container) {
    ...
    const bar = html.take(container).div.className('flex flex-wrap gap-4 items-end p-4 bg-white border border-gray-200 rounded-xl').ele();

    for (const f of this.filters) {
      if (f.type === 'select') {
        const grp = html.take(bar).div.className('flex flex-col gap-1').ele();
        ...
        const sel = html.take(grp)
          ...
          .dataAttr('ff', f.field)
          .ele();
        html.take(sel).option.prop('value', '').text(this.labels.all);
        for (const o of (f.options || [])) {
          const value = typeof o === 'object' ? (o.value ?? o.id ?? '') : o;
          const label = typeof o === 'object' ? (o.label ?? value) : o;
          const opt = html.take(sel).option.prop('value', String(value)).text(String(label)).ele();
        ...
        const grp     = html.take(bar).div.className('flex flex-col gap-1').ele();
        html.take(grp).label.className('text-xs font-medium text-gray-500 uppercase tracking-wide').text(f.label);
        const relWrap = html.take(grp).div.className('relative').ele();
        const searchIcon = html.take(relWrap).span.className('absolute inset-y-0 left-2.5 flex items-center text-gray-400 text-sm').ele();
        ...
        const grp       = html.take(bar).div.className('flex flex-col gap-1').ele();
        html.take(grp).label.className('text-xs font-medium text-gray-500 uppercase tracking-wide').text(f.label);
        const rangeWrap = html.take(grp).div.className('flex gap-2 items-center').ele();
        const inpFrom   = html.take(rangeWrap).input.type('text').className('px-2 py-1.5 text-sm border border-gray-300 rounded-md bg-white').dataAttr('ff', f.field + '_from').value(String(values[f.field + '_from'] || '')).ele();
        const separator = html.take(rangeWrap).span.className('text-gray-400 text-xs').ele();
        appendIcon(separator, 'arrow-right');
        const inpTo     = html.take(rangeWrap).input.type('text').className('px-2 py-1.5 text-sm border border-gray-300 rounded-md bg-white').dataAttr('ff', f.field + '_to').value(String(values[f.field + '_to'] || '')).ele();
    ...
    const clearBtn = html.take(bar).button.className('self-end px-3 py-1.5 text-sm text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-md transition-colors').text(this.labels.clear).ele();
