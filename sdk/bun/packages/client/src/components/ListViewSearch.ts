...
export class ListViewSearch extends BaseComponent {
  ...
  draw(container) {
    ...
    const wrap = html.take(container).div.className('flex flex-col gap-3').ele();

    const searchWrap = html.take(wrap).div.className('relative flex items-center').ele();
    const searchIcon = html.take(searchWrap).span.className('absolute left-2.5 text-gray-400 text-sm pointer-events-none select-none').ele();
    ...
    if (loading) {
      for (let i = 0; i < 3; i++) {
        const card = html.take(wrap).div.className('bg-white rounded-lg border border-gray-200 p-4 space-y-2 animate-pulse').ele();
    ...
    const list = html.take(wrap).div.className('flex flex-col gap-2').ele();
