...
export class ProgressBar extends BaseComponent {
  draw(container) {
    ...
    const card   = html.take(container).div.className('bg-white rounded-lg border border-gray-200 p-4').ele();
    const header = html.take(card).div.className('flex items-center justify-between mb-2').ele();
    ...
    const track = html.take(card).div.className('w-full bg-gray-100 rounded-full h-2.5').ele();
