...
export class SplitPanel extends BaseComponent {
  ...
  draw(container) {
    ...
    const grid = html.take(container).div.className('grid gap-6').style(`grid-template-columns: ${lw} ${rw}`).ele();
    if (this.leftComp)  this.leftComp.mount(html.take(grid).div.ele());
    if (this.rightComp) this.rightComp.mount(html.take(grid).div.ele());
  }
}
