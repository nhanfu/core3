...
export class CheckboxInput extends BaseComponent {
  ...
  draw(container) {
    ...
    const inp = html.take(lbl).input.type('checkbox').className('token-form-check w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500').getContext();
