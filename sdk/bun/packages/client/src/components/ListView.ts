...
export class ListView extends BaseComponent {
  ...
  private async drawOdoo(container: HTMLElement, version: number) {
    ...
    if (!rows.length) {
      ...
      } else if (empty.illustration === 'package') {
        const topMargin = window.innerWidth < 576 ? '90px' : '150px';
        const illustration = html.take(cell).div
          .className('o-list-empty-illustration')
          .css('width', '190px')
          ...
          .attr('aria-hidden', 'true')
          .ele();
        html.take(illustration).innerHTML('<svg viewBox="0 0 190 190" width="190" height="190" xmlns="http://www.w3.org/2000/svg"><circle cx="95" cy="124" r="61" fill="#6d486b"/><path d="M57 22h61l25 25v122H57z" fill="#fff" stroke="#adb5bd" stroke-width="3"/><path d="M118 22v25h25" fill="none" stroke="#adb5bd" stroke-width="3"/><path d="M69 121h53M69 133h53M69 145h38" stroke="#e9ecef" stroke-width="7" stroke-linecap="round"/><circle cx="84" cy="98" r="4" fill="#adb5bd"/><circle cx="106" cy="98" r="4" fill="#adb5bd"/><path d="M84 111q11 9 22 0" fill="none" stroke="#adb5bd" stroke-width="3" stroke-linecap="round"/><path d="M36 61l-10-10m10 23H22m18-33V34" stroke="#adb5bd" stroke-width="3" stroke-linecap="round"/></svg>');
      }
