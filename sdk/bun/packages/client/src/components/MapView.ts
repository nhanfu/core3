...
export class MapView extends BaseComponent {
  ...
  draw(container: HTMLElement) {
    ...
    rows.forEach((row, index) => { const marker = html.take(map).button.type('button').className('o-map-marker').dataAttr('row-id', String(row[this.options.rowKey || 'id'] ?? index)).attr('title', String(row[view.labelField] ?? '')).text(String(index + 1)).getContext() as HTMLButtonElement; if (hasCoordinates) { const lat = Number(row[view.latitudeField!]); const lon = Number(row[view.longitudeField!]); marker.style.left = `${Math.min(94, Math.max(4, (lon + 180) / 360 * 100))}%`; marker.style.top = `${Math.min(94, Math.max(4, (90 - lat) / 180 * 100))}%`; } else { marker.style.left = `${8 + (index % 8) * 12}%`; marker.style.top = `${18 + Math.floor(index / 8) * 16}%`; } if (this.options.openAction) html.take(marker).event('click', () => void this.submit(this.options.openAction!, { row })); });
