// @core3 — runtime contracts (script execution, page config, event bus)
...
export type BuiltInRuntime =
  | 'javascript'    // V8 isolate (vm2 / isolated-vm)
  | 'typescript'    // V8 isolate + ts-strip
  | 'python'        // Subprocess / Pyodide (WASM)
  | 'lua'           // Fengari (WASM)
  | 'shell';        // Subprocess (sandboxed)

// ─── Page configuration (parsed from YAML) ───────────────────────────────────
...
export interface FrameworkConfig {
  ...
  runtimes?: RuntimeAdapter[];    // additional script runtimes
