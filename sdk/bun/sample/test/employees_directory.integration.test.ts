...
describe('Employees Directory parity', () => {
  ...
  test('keeps public fixtures deterministic and explicit across filtered states', async () => {
    ...
    const defaults = { q: null, active: 'active', company_name: null, current_company_name: 'Core3 Vietnam', department_name: null, manager_name: null, newly_hired: null, fixture_state: null };
    ...
    expect((await repository.querySource(detail, { id: 'employee-demo-002', current_company_name: 'Core3 Vietnam', fixture_state: null }, 0, 1)).data).toMatchObject({ name: 'Nguyen Minh Anh', work_email: 'anh.nguyen@core3.local' });
    expect((await repository.querySource(detail, { id: 'employee-demo-005', current_company_name: 'Core3 Vietnam', fixture_state: null }, 0, 1)).data).toEqual({});
    expect((await repository.querySource(detail, { id: 'employee-demo-002', current_company_name: 'Core3 Vietnam', fixture_state: 'not_found' }, 0, 1)).data).toEqual({});
    database.close();
  });
});
