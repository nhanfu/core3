...
describe('Timesheets All Timesheets Billed on Timesheets filter parity', () => {
  test('maps Odoo sale_timesheet Billed on Timesheets to paired page/API contracts', () => {
    ...
    expect(odoo).toContain('<filter name="billable_time" string="Billed on Timesheets" domain="[(\'timesheet_invoice_type\', \'=\', \'billable_time\')]"');
    ...
    expect(list.filters).toContainEqual({ field: 'billing_type', label: 'Billed on Timesheets', options: [{ id: 'billable_time', label: 'Billed on Timesheets' }] });
    ...
    expect(String(source.query)).toContain("'billable_time'");
  });

  test('filters durable rows by billable timesheet type while preserving company, permission, and empty guards', async () => {
    const database = await DuckDbDatabase.open(':memory:');
    const repository = new YamlRepository(database);
    await migrateDatabase(repository, join(serviceRoot, 'migrations'), undefined, 'timesheets_all_billed_on_timesheets_filter', ['schema', 'data']);
    ...
    expect(filtered.data.every((row: any) => row.billing_type === 'billable_time')).toBe(true);
  ...
  test('reflects a concurrent billing-type change instead of retaining stale filtered rows', async () => {
    const database = await DuckDbDatabase.open(':memory:');
    const repository = new YamlRepository(database);
    await migrateDatabase(repository, join(serviceRoot, 'migrations'), undefined, 'timesheets_all_billed_on_timesheets_filter_concurrency', ['schema', 'data']);
    const source = yaml('api/all-timesheets.yaml').datasources.find((item: any) => item.id === 'all_timesheet_entries');

    const before = await repository.querySource(source, valid, 0, 50);
    expect(before.data.length).toBeGreaterThan(0);
    const changedId = before.data[0].id;
    await repository.query("UPDATE timesheet_entries SET billing_type = 'non_billable', billable = FALSE, row_version = row_version + 1 WHERE id = ?", [changedId]);
    expect((await repository.querySource(source, valid, 0, 50)).data.some((row: any) => row.id === changedId)).toBe(false);
    expect((await repository.querySource(source, { ...valid, billing_type: 'non_billable' }, 0, 50)).data.some((row: any) => row.id === changedId)).toBe(true);
    database.close();
  });

  test('retains Billed on Timesheets filtering after a file-backed restart', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'core3-timesheets-all-billed-on-timesheets-filter-'));
    ...
    try {
      ...
      await migrateDatabase(repository, migrations, undefined, 'timesheets_all_billed_on_timesheets_filter_restart', ['schema', 'data']);
      const initial = await repository.querySource(source, valid, 0, 50);
      expect(initial.data.length).toBeGreaterThan(0);
      expect(initial.data.every((row: any) => row.billing_type === 'billable_time')).toBe(true);
      first.close();

      const second = await DuckDbDatabase.open(path);
      const reopened = new YamlRepository(second);
      await migrateDatabase(reopened, migrations, undefined, 'timesheets_all_billed_on_timesheets_filter_restart', ['schema', 'data']);
