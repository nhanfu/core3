...
describe('Inventory receipts and deliveries transfer workflow parity', () => {
  test('keeps list/detail layout and API ownership aligned with page ids and Odoo view order', () => {
    ...
    expect(detail.header_actions.map((candidate: any) => candidate.label)).toEqual(['Edit details', 'Delete', 'Mark as Todo', 'Check Availability', 'Put in Pack', 'Unreserve', 'Validate', 'Create Backorder', 'Return', 'Cancel']);
