...
describe('Purchase order line editor parity', () => {
  test('binds the Odoo order form and x2many line surface through page-id API fragments', () => {
    ...
    expect(form.header_actions.map((entry: any) => entry.label)).toEqual(['Send RFQ', 'Send PO', 'Send Reminder', 'Confirm Order', 'Approve Order', 'Receive', 'Acknowledge', 'Lock', 'Unlock', 'Print', 'Print', 'Cancel', 'Set to Draft']);
