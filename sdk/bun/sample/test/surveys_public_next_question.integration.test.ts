...
describe('Surveys public next-question navigation', () => {
  test('keeps Odoo next-question route, page/API ownership, public permission, and guards explicit', () => {
    ...
    expect(next.mutation.guards.map((guard: any) => guard.code)).toEqual(['SURVEY_PUBLIC_NEXT_STALE', 'SURVEY_PUBLIC_RESPONSE_EXPIRED', 'SURVEY_PUBLIC_TIME_LIMIT_EXPIRED', 'SURVEY_PUBLIC_NEXT_INVALID']);
