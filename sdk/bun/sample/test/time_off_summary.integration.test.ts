...
describe('Time Off Summary report action parity', () => {
  test('keeps the Odoo employee Print action contract in the page-id API seam', () => {
    ...
    expect(action).toMatchObject({
      ...
      submit_label: 'Print', cancel_label: 'Cancel', modal_style: 'time_off_summary', handler: 'yaml_mutation', operation: 'print_report',
    });
    ...
    expect(action.mutation.guards.map((guard: any) => guard.status)).toEqual([422, 404, 422, 422]);
  });
