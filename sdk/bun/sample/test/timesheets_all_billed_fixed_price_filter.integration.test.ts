...
describe('Timesheets All Timesheets Billed at a Fixed Price filter parity', () => {
  test('maps Odoo sale_timesheet Billed at a Fixed Price to paired page/API contracts', () => {
  ...
  test('filters durable rows by fixed-price billing while preserving company, permission, and empty guards', async () => {
    const database = await DuckDbDatabase.open(':memory:');
    const repository = new YamlRepository(database);
    await migrateDatabase(repository, join(serviceRoot, 'migrations'), undefined, 'timesheets_all_billed_fixed_price_filter', ['schema', 'data']);
    ...
    expect(filtered.data.every((row: any) => row.billing_type === 'billable_fixed')).toBe(true);
  ...
  test('reflects a concurrent billing-type change instead of retaining stale filtered rows', async () => {
    const database = await DuckDbDatabase.open(':memory:');
    const repository = new YamlRepository(database);
    await migrateDatabase(repository, join(serviceRoot, 'migrations'), undefined, 'timesheets_all_billed_fixed_price_filter_concurrency', ['schema', 'data']);
    const source = yaml('api/all-timesheets.yaml').datasources.find((item: any) => item.id === 'all_timesheet_entries');
  ...
  test('retains Billed at a Fixed Price filtering after a file-backed restart', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'core3-timesheets-all-billed-fixed-price-filter-'));
    ...
    try {
      ...
      await migrateDatabase(repository, migrations, undefined, 'timesheets_all_billed_fixed_price_filter_restart', ['schema', 'data']);
      const initial = await repository.querySource(source, valid, 0, 50);
      expect(initial.data.length).toBeGreaterThan(0);
      expect(initial.data.every((row: any) => row.billing_type === 'billable_fixed')).toBe(true);
      first.close();

      const second = await DuckDbDatabase.open(path);
      const reopened = new YamlRepository(second);
      await migrateDatabase(reopened, migrations, undefined, 'timesheets_all_billed_fixed_price_filter_restart', ['schema', 'data']);
