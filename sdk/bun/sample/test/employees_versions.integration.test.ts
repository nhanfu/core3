...
describe('Employees Employee Records bounded parity', () => {
  ...
  test('seeds deterministic relation-backed records and report states idempotently', async () => {
    ...
    const defaults = { q: null, active: null, contract_state: null, employee_id: null, current_company_name: null, fixture_state: null };
    expect((await repository.querySource(source, defaults, 0, 50)).data.map((row: any) => row.id)).toEqual(['employee-version-admin-future', 'employee-version-admin-current', 'employee-version-anh-expired', 'employee-version-anh-archived']);
    expect((await repository.querySource(source, { ...defaults, employee_id: 'employee-demo-001' }, 0, 50)).data.map((row: any) => row.employee_id)).toEqual(['employee-demo-001', 'employee-demo-001']);
    expect((await repository.querySource(source, { ...defaults, current_company_name: 'Other Company' }, 0, 50)).data).toEqual([]);
    expect((await repository.querySource(source, { ...defaults, contract_state: 'future' }, 0, 50)).data).toMatchObject([{ employee_name: 'Admin User', contract_state: 'future' }]); expect((await repository.querySource(source, { ...defaults, active: 'true', fixture_state: 'empty' }, 0, 50)).data).toEqual([]); await expect(repository.querySource(source, { ...defaults, fixture_state: 'transport_error' }, 0, 50)).rejects.toMatchObject({ status: 503, code: 'EMPLOYEES_VERSIONS_UNAVAILABLE' }); await database.close();
  });
});
