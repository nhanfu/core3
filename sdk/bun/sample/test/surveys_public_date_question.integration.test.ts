...
const requiredAnswers = {
  ...
  'question-certification-date': '2026-01-15',
};
...
describe('Surveys public Date question', () => {
  test('keeps the Odoo date source and the paired page/API contract', () => {
    ...
    expect(renderer).toContain('YYYY-MM-DD');
    expect(renderer).toContain('isIsoDate');
    expect(yaml('migrations/20260923000000-026-survey-public-date-question.yaml').version).toBe('0.0.26');
  });

  test('rejects invalid ISO dates without mutation, persists a valid date across restart, and replays one submit', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'core3-surveys-public-date-'));
    const databasePath = join(directory, 'surveys.duckdb');
    const migrationTable = 'surveys_public_date_question_restart';
    ...
    expect((await landing.json()).questions).toContainEqual(expect.objectContaining({ id: 'question-certification-date', question_type: 'Date' }));
    const started = await route(`/api/public/surveys/${surveyToken}/start`, { method: 'POST', headers, body: JSON.stringify({ idempotency_key: 'public-date-start-001' }) });
    expect(started.status).toBe(200);
    const answerToken = (await started.json()).answer.access_token;
    await repository.run("UPDATE survey_responses SET current_question_id = 'question-certification-date' WHERE access_token = ?", [answerToken]);

    const invalid = await route(`/api/public/surveys/${surveyToken}/progress`, {
      method: 'POST',
      headers,
      body: JSON.stringify({ answer_token: answerToken, answers: { 'question-certification-date': '2026-02-30' } }),
    });
    expect(invalid.status).toBe(422);
    expect(await invalid.json()).toEqual({ error: 'Invalid answers: When did you complete the vendor training?', code: 'SURVEY_PUBLIC_ANSWER_INVALID' });
    ...
    const valid = await route(`/api/public/surveys/${surveyToken}/progress`, {
      method: 'POST',
      headers,
      body: JSON.stringify({ answer_token: answerToken, answers: { 'question-certification-date': '2026-01-15' } }),
    });
    expect(valid.status).toBe(200);
    expect((await valid.json()).answer.answer_data).toBe('{"question-certification-date":"2026-01-15"}');
    database.close();
    ...
    expect(await reopenedRepository.query('SELECT state, answer_data, current_question_id FROM survey_responses WHERE access_token = ?', [answerToken])).toEqual([
      { state: 'In Progress', answer_data: '{"question-certification-date":"2026-01-15"}', current_question_id: 'question-certification-date' },
    ]);

    const body = JSON.stringify({ answer_token: answerToken, answers: requiredAnswers, idempotency_key: 'public-date-submit-001' });
    ...
    expect(await reopenedRepository.query("SELECT COUNT(*) AS count FROM survey_responses WHERE idempotency_key = 'public-date-submit-001'")).toEqual([{ count: 1 }]);
    expect((await reopenedRepository.query('SELECT state, answer_data FROM survey_responses WHERE access_token = ?', [answerToken]))[0]).toMatchObject({ state: 'Submitted', answer_data: expect.stringContaining('question-certification-date') });
    ...
    const wrongToken = await reopenedRoute(`/api/public/surveys/${surveyToken}/progress`, { method: 'POST', headers, body: JSON.stringify({ answer_token: '00000000-0000-0000-0000-000000000000', answers: { 'question-certification-date': '2026-01-15' } }) });
