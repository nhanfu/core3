...
describe('Surveys migration replay and rollback', () => {
  ...
  test('upgrades after rollback and replay remains stable', async () => {
    ...
    expect(await repository.query(`SELECT version FROM ${table} ORDER BY version`)).toHaveLength(21);
  ...
  test('rolls the complete chain down and back up on DuckDB', async () => {
    ...
    await migrateDatabase(repository, migrations, undefined, table, ['schema', 'data']);
    expect(await repository.query(`SELECT COUNT(*) AS count FROM ${table}`)).toEqual([{ count: 21 }]);
