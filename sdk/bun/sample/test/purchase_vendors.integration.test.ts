...
describe('Purchase Vendors parity', () => {
  test('binds the inherited Odoo Vendors list/detail action through page-id API fragments', () => {
    ...
    expect(listView.views[1]).toMatchObject({ label: 'Kanban', mobile: true, card: { title: 'name', subtitle: 'email' } });
  ...
  test('serves deterministic active and archived supplier fixtures with search, empty, and transport states', async () => {
    ...
    expect(active.data.find((row: any) => row.name === 'Northwind Components')).toMatchObject({ purchase_order_count: 3, open_purchase_order_count: 3, country: 'United States' });
