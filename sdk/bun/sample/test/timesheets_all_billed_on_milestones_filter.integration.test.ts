...
const valid = {
  ...
  task_id: null,
  mine: null,
  billing_type: 'billable_milestones',
  state: null,
  work_date: null,
  fixture_state: null,
...
describe('Timesheets All Timesheets Billed on Milestones filter parity', () => {
  test('maps Odoo sale_timesheet Billed on Milestones to paired page/API contracts', () => {
    ...
    expect(odoo).toContain('<filter name="billable_milestones" string="Billed on Milestones" domain="[(\'timesheet_invoice_type\', \'=\', \'billable_milestones\')]"');
    ...
    expect(list.filters).toContainEqual({ field: 'billing_type', label: 'Billed on Milestones', options: [{ id: 'billable_milestones', label: 'Billed on Milestones' }] });
    expect(source).toMatchObject({ id: 'all_timesheet_entries', permission: 'timesheets.manage', workflow: 'timesheet_entries' });
    expect(source.meta.filter_contracts).toContainEqual({ field: 'billing_type', value: 'billable_milestones', label: 'Billed on Milestones', source: 'timesheet_entries.billing_type' });
    expect(source.pivot.fields).toContain('billing_type');
    expect(String(source.query)).toContain(':billing_type');
  });

  test('filters durable rows by milestone billing while preserving company, permission, and empty guards', async () => {
    const database = await DuckDbDatabase.open(':memory:');
    const repository = new YamlRepository(database);
    await migrateDatabase(repository, join(serviceRoot, 'migrations'), undefined, 'timesheets_all_billed_on_milestones_filter', ['schema', 'data']);
    ...
    expect(filtered.data.every((row: any) => row.billing_type === 'billable_milestones')).toBe(true);
  ...
  test('reflects a concurrent billing-type change instead of retaining stale filtered rows', async () => {
    const database = await DuckDbDatabase.open(':memory:');
    const repository = new YamlRepository(database);
    await migrateDatabase(repository, join(serviceRoot, 'migrations'), undefined, 'timesheets_all_billed_on_milestones_filter_concurrency', ['schema', 'data']);
    const source = yaml('api/all-timesheets.yaml').datasources.find((item: any) => item.id === 'all_timesheet_entries');

    const before = await repository.querySource(source, valid, 0, 50);
    expect(before.data.length).toBeGreaterThan(0);
    const changedId = before.data[0].id;
    await repository.query("UPDATE timesheet_entries SET billing_type = 'billable_manual', billable = TRUE, row_version = row_version + 1 WHERE id = ?", [changedId]);
  ...
  test('retains Billed on Milestones filtering after a file-backed restart', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'core3-timesheets-all-billed-on-milestones-filter-'));
    ...
    try {
      ...
      await migrateDatabase(repository, migrations, undefined, 'timesheets_all_billed_on_milestones_filter_restart', ['schema', 'data']);
      const initial = await repository.querySource(source, valid, 0, 50);
      expect(initial.data.length).toBeGreaterThan(0);
      expect(initial.data.every((row: any) => row.billing_type === 'billable_milestones')).toBe(true);
      first.close();

      const second = await DuckDbDatabase.open(path);
      const reopened = new YamlRepository(second);
      await migrateDatabase(reopened, migrations, undefined, 'timesheets_all_billed_on_milestones_filter_restart', ['schema', 'data']);
