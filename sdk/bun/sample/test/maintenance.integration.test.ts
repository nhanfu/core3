...
describe('Maintenance bounded Odoo parity batch', () => {
  ...
  test('exposes Odoo request statusbar actions and cancel action', () => {
    ...
    expect(form.header_actions).toEqual([
      ...
      { id: 'update_maintenance_request_instructions', label: 'Edit instructions', variant: 'secondary', permission: 'maintenance.write', show_if: "state.maintenance_request_detail.archived === false" },
    ]);
    ...
    expect(api.actions.map((action: any) => action.id)).toEqual([
      ...
      'update_maintenance_request_instructions',
