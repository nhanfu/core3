...
describe('Events attendee parity batch', () => {
  ...
  test('matches the Odoo event form actions and stat buttons', () => {
    ...
    expect(form.header_actions.map((action: any) => action.id)).toEqual([
      ...
      'edit_event_detail',
      'schedule_event_detail_activity',
      'delete_event_detail',
  ...
  test('declares the event detail ticket, question, and slot tabs', () => {
    ...
    expect(yaml('api/event-detail.yaml').datasources.map((source: any) => source.id)).toEqual([
      'event_detail', 'event_registrations', 'event_detail_tags', 'event_detail_tag_options', 'event_tickets', 'event_detail_questions', 'event_detail_question_options', 'event_detail_slots', 'event_detail_communications', 'event_badge_background', 'event_detail_activities', 'event_detail_chatter', 'event_followers', 'event_follower_candidates',
    ]);
  ...
  test('matches the Odoo attendee form sections and lifecycle controls', () => {
    ...
    expect(form.header_actions.map((action: any) => action.id)).toEqual(['print_attendee_badge', 'print_attendee_full_page_ticket', 'print_attendee_responsive_html_ticket', 'send_attendee_email', 'edit_event_attendee', 'confirm_attendee_detail', 'mark_attendee_attended_detail', 'cancel_event_attendee_detail', 'reopen_event_attendee_detail']);
  });
