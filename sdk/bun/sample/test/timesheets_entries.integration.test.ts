...
describe('Timesheets entry CRUD parity slice', () => {
  ...
  test('is idempotent and persists valid create, update, and delete', async () => {
    ...
    const created = await repository.executeMutation(create.mutation, { current_company_name: 'Core3 Demo Company', current_user_name: values.employee_name, values }) as any;
    ...
    const updated = await repository.executeMutation(edit.mutation, { id: created.id, expected_row_version: 1, current_company_name: 'Core3 Demo Company', current_user_name: values.employee_name, values: { ...values, description: 'Updated QA implementation', hours: 3 } }) as any;
  ...
  test('rejects invalid relations, amounts, locked records, and stale writes', async () => {
    ...
    const created = await repository.executeMutation(create.mutation, { current_company_name: 'Core3 Demo Company', current_user_name: values.employee_name, values: { ...values, name: 'TS/2026/QA-LOCK' } }) as any;
    const edit = action('api/entry-detail.yaml', 'edit_timesheet_detail');
    await expect(repository.executeMutation(edit.mutation, { id: created.id, expected_row_version: 99, current_company_name: 'Core3 Demo Company', current_user_name: values.employee_name, values })).rejects.toMatchObject({ status: 409, code: 'STALE_RECORD' });
    const submit = yaml('pages/timesheet-workflow.yaml').workflow.transitions.find((transition: any) => transition.id === 'submit');
    await repository.executeMutation(submit.mutation, { id: created.id, current_company_name: 'Core3 Demo Company', current_user_name: values.employee_name });
    await expect(repository.executeMutation(edit.mutation, { id: created.id, expected_row_version: 2, current_company_name: 'Core3 Demo Company', current_user_name: values.employee_name, values })).rejects.toMatchObject({ status: 409, code: 'TIMESHEET_ENTRY_LOCKED' });
  });
});
