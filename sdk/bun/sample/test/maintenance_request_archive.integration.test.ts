...
describe('Maintenance Request cancel and reopen parity', () => {
  test('binds Odoo Cancel and Reopen Request to page-owned APIs', () => {
    ...
    expect(page.components[0].header_actions.map((action: any) => action.label)).toEqual(['Cancel', 'Reopen Request', 'Update kanban state', 'Schedule activity', 'Generate next occurrence', 'Edit instructions']);
