...
describe('Recruitment parity batch', () => {
  it('discovers service-owned API fragments for the core Odoo states', () => {
    ...
    expect(discovered.pages.get('applicants')?.config.components[0].views.map((view: any) => view.id)).toEqual(['list', 'activity', 'calendar', 'kanban']);
  ...
  it('seeds deterministic report rows and supports search, empty, and transport-error states', async () => {
    ...
    expect(ready.data.map((row: any) => row.stage)).toEqual(['New', 'New', 'New', 'Screening', 'Screening', 'Interview']);
