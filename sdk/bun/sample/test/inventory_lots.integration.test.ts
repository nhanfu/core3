...
describe('Inventory Lots / Serial Numbers Odoo action parity', () => {
  ...
  test('seeds idempotent lots, search/filter/detail, empty and transport-error states', async () => {
    ...
    expect((await repository.querySource(lots, { q: null, availability: null, fixture_state: null }, 0, 50)).data).toHaveLength(11);
    ...
    expect((await repository.querySource(lots, { q: null, availability: 'on_hand', fixture_state: null }, 0, 50)).data.map((row: any) => row.name)).toEqual(['CUST-LOT-0001', 'CAB-LOT-0002', 'TRACE-LOT-0001', 'CM-BOX-00001', 'CM-BOX-00002', 'CAB-LOT-0001', 'TRACE-TRANSFER-0001', '0000000000029', 'T0001']);
