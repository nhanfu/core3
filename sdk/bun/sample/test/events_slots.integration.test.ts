...
describe('Events event slots parity', () => {
  test('binds the event-scoped list/form workflow through page IDs', () => {
    ...
    expect(slots).toMatchObject({ source: 'event_detail_slots', create_action: 'create_event_slot', row_open_action: 'view_event_slot', row_double_click_action: 'view_event_slot', search: { label: 'Search slots', placeholder: 'Search slots...' } });
    ...
    expect(source('event-detail.yaml', 'event_detail_slots')).toMatchObject({ permission: 'events.read', error_states: { transport_error: { status: 503, code: 'EVENT_SLOTS_UNAVAILABLE' } } });
  ...
  test('seeds deterministic slot availability and guards CRUD boundaries', async () => {
    ...
    const list = await repository.querySource(source('event-detail.yaml', 'event_detail_slots'), { id: 'event-demo-slot-20260115', fixture_state: null, q: null }, 0, 50);
    expect(list.data).toHaveLength(1);
    expect(list.data[0]).toMatchObject({ id: 'slot-event-20260115-main', name: 'Main session', row_version: 1, date_tz: 'Europe/Brussels', color: '#875A7B', start_at: expect.stringContaining('2026-01-15') });
    expect((await repository.querySource(source('event-detail.yaml', 'event_detail_slots'), { id: 'event-demo-slot-20260115', fixture_state: null, q: 'missing' }, 0, 50)).data).toEqual([]);

    const detail = source('event-slot-detail.yaml', 'event_slot_detail');
    expect(await repository.querySource(detail, { id: 'slot-event-20260115-main', fixture_state: null }, 0, 1)).toMatchObject({ data: { event_name: 'Event Slot Planning Session', availability: '39 seats remaining' } });
    expect((await repository.querySource(detail, { id: 'does-not-exist', fixture_state: 'not_found' }, 0, 1)).data).toEqual({});
    expect((await repository.querySource(detail, { id: 'slot-event-20260115-main', fixture_state: 'empty' }, 0, 1)).data).toEqual({});
    ...
    const created = await repository.executeMutation(create.mutation, { values: {
      event_id: 'event-demo-slot-20260115', name: 'Closing session', start_at: '2026-01-15 13:00:00', end_at: '2026-01-15 14:00:00', seats: 25, date_tz: 'Europe/Brussels', color: '#00A09D',
    } });
    ...
    await expect(repository.executeMutation(create.mutation, { values: {
      event_id: 'event-demo-slot-20260115', name: 'Outside event', start_at: '2026-01-14 17:45:00', end_at: '2026-01-14 18:30:00', seats: 25,
    } })).rejects.toMatchObject({ status: 422, code: 'EVENT_SLOT_OUTSIDE_EVENT' });
    await expect(repository.executeMutation(create.mutation, { values: {
      event_id: 'event-demo-slot-20260115', name: 'Invalid range', start_at: '2026-01-15 14:30:00', end_at: '2026-01-15 14:00:00', seats: 25,
    ...
    const updated = await repository.executeMutation(update.mutation, { id: created.id, expected_row_version: 1, values: {
      name: 'Closing session updated', start_at: '2026-01-15 13:00:00', end_at: '2026-01-15 14:10:00', seats: 30, date_tz: 'Europe/Brussels', color: '#714B67',
    } });
    expect(updated).toMatchObject({ id: created.id, name: 'Closing session updated', seats: 30, row_version: 2 });
    await expect(repository.executeMutation(update.mutation, { id: created.id, expected_row_version: 1, values: { name: 'Stale slot', start_at: '2026-01-15 13:00:00', end_at: '2026-01-15 14:00:00', seats: 30 } })).rejects.toMatchObject({ status: 409, code: 'STALE_RECORD' });

    await expect(repository.executeMutation(remove.mutation, { id: 'slot-event-20260115-main', expected_row_version: 1 })).rejects.toMatchObject({ status: 409, code: 'EVENT_SLOT_HAS_REGISTRATIONS' });
