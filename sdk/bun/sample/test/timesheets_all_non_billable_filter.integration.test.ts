...
describe('Timesheets All Timesheets Non-Billable filter parity', () => {
  test('maps Odoo sale_timesheet Non-Billable to paired page/API contracts', () => {
    ...
    expect(odoo).toContain('<filter name="non_billable" string="Non-Billable" domain="[(\'timesheet_invoice_type\', \'=\', \'non_billable\')]"');
    ...
    expect(list.filters).toContainEqual({ field: 'billing_type', label: 'Non-Billable', options: [{ id: 'non_billable', label: 'Non-Billable' }] });
  ...
  test('filters durable rows by billing type while preserving company, permission, and empty guards', async () => {
    const database = await DuckDbDatabase.open(':memory:');
    const repository = new YamlRepository(database);
    await migrateDatabase(repository, join(serviceRoot, 'migrations'), undefined, 'timesheets_all_non_billable_filter', ['schema', 'data']);
    ...
    expect(filtered.data.every((row: any) => row.billing_type === 'non_billable')).toBe(true);
  ...
  test('reflects a concurrent billing-type change instead of retaining stale filtered rows', async () => {
    const database = await DuckDbDatabase.open(':memory:');
    const repository = new YamlRepository(database);
    await migrateDatabase(repository, join(serviceRoot, 'migrations'), undefined, 'timesheets_all_non_billable_filter_concurrency', ['schema', 'data']);
    const source = yaml('api/all-timesheets.yaml').datasources.find((item: any) => item.id === 'all_timesheet_entries');
  ...
  test('retains Non-Billable filtering after a file-backed restart', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'core3-timesheets-all-non-billable-filter-'));
    ...
    try {
      ...
      await migrateDatabase(repository, migrations, undefined, 'timesheets_all_non_billable_filter_restart', ['schema', 'data']);
      const initial = await repository.querySource(source, valid, 0, 50);
      expect(initial.data.length).toBeGreaterThan(0);
      expect(initial.data.every((row: any) => row.billing_type === 'non_billable')).toBe(true);
      first.close();

      const second = await DuckDbDatabase.open(path);
      const reopened = new YamlRepository(second);
      await migrateDatabase(reopened, migrations, undefined, 'timesheets_all_non_billable_filter_restart', ['schema', 'data']);
