...
describe('Sales customer reference boundary', () => {
  test('keeps page/API joins and permissioned guards explicit', () => {
    ...
    for (const definition of [quotationApi, orderApi, detailApi, templateApi]) {
      const mutationActions = definition.actions.filter((entry: any) => entry.mutation?.table === 'orders' && ['insert', 'update'].includes(entry.mutation.operation) && !['import_ecommerce_sales_order', 'apply_sale_quotation_template'].includes(entry.id));
