...
describe('Surveys public previous-question navigation', () => {
  test('keeps the Odoo back-navigation source, YAML action, permission, and renderer binding explicit', () => {
    ...
    expect(previous.mutation.guards.map((guard: any) => guard.code)).toEqual(['SURVEY_PUBLIC_PREVIOUS_STALE', 'SURVEY_PUBLIC_RESPONSE_EXPIRED', 'SURVEY_PUBLIC_PREVIOUS_INVALID']);
