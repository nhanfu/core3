...
## 2026-09-21 — `TIMESHEET-ALL-BILLED-ON-TIMESHEETS-FILTER-001`

- Selected the next uncovered source-backed All Timesheets interaction: Odoo `sale_timesheet` adds the `Billed on Timesheets` search filter over `timesheet_invoice_type = billable_time`, distinct from Non-Billable and the completed Sales Order/report slices.
- Kept `pages/all-timesheets.yaml` and `api/all-timesheets.yaml` separate and joined by `page.id: all-timesheets`; the page adds the filter and the API exposes durable `billing_type` in the pivot contract and retains manager permission plus company/empty guards.
- Focused verification: `test/timesheets_all_billed_on_timesheets_filter.integration.test.ts` passed 4/4 tests with 21 expectations, including source mapping, durable filter reads, relation-update freshness, permission/company/empty guards, and file-backed restart.
...
## 2026-09-21 — `TIMESHEET-ALL-BILLED-ON-MILESTONES-FILTER-001`

- Selected the next uncovered source-backed All Timesheets interaction: Odoo `sale_timesheet` adds the `Billed on Milestones` search filter over `timesheet_invoice_type = billable_milestones`, distinct from the completed billing filters.
- Kept `pages/all-timesheets.yaml` and `api/all-timesheets.yaml` separate and joined by `page.id: all-timesheets`; the page adds the filter and the API records its explicit durable `billing_type` contract, exposes pivot billing type, and retains manager permission plus company/empty guards.
- Focused verification: `test/timesheets_all_billed_on_milestones_filter.integration.test.ts` passed 4/4 tests with 21 expectations, including source mapping, durable filter reads, relation-update freshness, permission/company/empty guards, and file-backed restart.
- Authenticated Odoo evidence is under `evidence/timesheets/2026-09-21/timesheet-all-billed-on-milestones-filter/`: desktop applies Billed on Milestones; mobile captures responsive Kanban; `odoo-results.json` records no page/request errors.
- Core3 desktop/mobile evidence is blocked before authentication because backend `127.0.0.1:3001/api/modules` did not become available during the bounded startup probe; exact output is in `core3-readiness.txt`. Odoo Print/PDF/report-action blockers remain open and this slice is not sign-off.
- Repository audit remains blocked by the unrelated shared page-schema boundary; exact output is recorded in `audit-blocker.txt`.

## 2026-09-21 — `TIMESHEET-ALL-BILLED-MANUALLY-FILTER-001`

- Selected the next uncovered source-backed All Timesheets interaction: Odoo `sale_timesheet` adds the `Billed Manually` search filter over `timesheet_invoice_type = billable_manual`, distinct from the completed billing filters.
- Kept `pages/all-timesheets.yaml` and `api/all-timesheets.yaml` separate and joined by `page.id: all-timesheets`; the page adds the filter and the API records its explicit durable `billing_type` contract, exposes pivot billing type, and retains manager permission plus company/empty guards.
- Focused verification: `test/timesheets_all_billed_manually_filter.integration.test.ts` passed 4/4 tests with 21 expectations, including source mapping, durable filter reads, relation-update freshness, permission/company/empty guards, and file-backed restart.
- Authenticated Odoo evidence is under `evidence/timesheets/2026-09-21/timesheet-all-billed-manually-filter/`: desktop applies Billed Manually; mobile captures responsive Kanban; `odoo-results.json` records no page/request errors.
- Core3 desktop/mobile evidence is blocked before authentication because backend `127.0.0.1:3001/api/modules` did not become available during the bounded startup probe; exact output is in `core3-readiness.txt`. Odoo Print/PDF/report-action blockers remain open and this slice is not sign-off.
...
- Focused coverage passed 4/4 tests with 21 expectations, including Odoo action/source mapping, page/API separation, permission/company/empty guards, durable create, stale context rejection, migration replay, and file-backed restart. Related project report/dashboard/dependency data coverage passed 18 tests with 101 expectations; the remaining project contract discovery test is blocked by a shared non-Timesheets YAML parse error.
...
## 2026-09-21 — `TIMESHEET-TASK-ACTION-DISPLAY-NAME-001`

- Selected the next open source-backed task behavior: Odoo
  `timesheet_action_task` names the record-context action `Task's Timesheets`
  and scopes it to task `active_ids`. This is separate from completed task and
  subtask row-scope behavior.
- Added the durable `task_timesheet_action_context` API datasource and a task
  page `StatRow` bound through `page.id: task-timesheets`; page and API YAML
  remain separated.
- Current-company, missing, empty, and read-permission boundaries are
  explicit. Existing durable task storage makes the title restart-safe without
  a new migration.
- Focused coverage passed 3/3 tests with 17 expectations; related task,
  report, and subtask coverage passed 14 tests with 83 expectations.
...
## 2026-09-21 — `TIMESHEET-TASK-ACTION-KANBAN-VIEW-001`

- Selected the Kanban branch of Odoo `project.task.action_view_subtask_timesheet`,
  which preserves `kanban` for the internal task action. This does not repeat
  the completed graph, project-context, multi-scope, display-name, or earlier
  Timesheets slices.
- Added a responsive employee-grouped Kanban view to the layout-only
  `task-timesheets` page. It remains paired with the durable
  `task_timesheet_entries` API by `page.id: task-timesheets`; the replay-safe
  migration `20260921190000-024-timesheets-task-action-kanban.yaml` adds the
  task/company/status/date lookup index.
- Focused coverage passed 4/4 tests with 17 expectations. Related task/action/
  report coverage passed 34/34 tests with 187 expectations. Scoped ESLint,
  UI audit passed at 756 pages / 765 routes / 1,534 datasources, and the
  Timesheets-owned staged diff-check passed.
