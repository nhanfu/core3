...
number of products in the Shop grid, defaults it to 21, and its Website Builder
`SetPpgAction` rejects values below 1 and clamps the persisted value to 10,000.
This is distinct from the completed Shop grid-column, page-container,
visibility, and default-sort slices.

Core3 migrations 142/143 add a durable company-scoped page-size policy and
deterministic My Company fixture. Separate
`api/shop-page-size-policy.yaml` and `pages/shop-page-size-policy.yaml`
contracts join by `ecommerce-shop-page-size-policy`; the update requires
`ecommerce.write`, validates 1–10,000, and uses company and row-version
guards. The Shop API/page expose the effective company setting and the
configuration route is manifest-owned. Focused integration tests cover missing
fixture reads, permission declarations, invalid/foreign/stale writes,
idempotent migration/fixture replay, and DuckDB restart persistence.

Evidence is under
`evidence/ecommerce/2026-09-21/ecom-catalog-shop-page-size-001/`.
Authenticated Core3 desktop/mobile capture is blocked by unavailable ports
3000/4312/4313; Odoo `/shop` is an exact HTTP 404 on 8069/8073. This bounded
slice is not Ecommerce module sign-off.
...
order setting. The supplied `website` model defines `product_page_cols_order`
as Regular order or Inverse order; the product template applies the inverse
choice as `flex-lg-row-reverse` on the main product-page row. This is distinct
from the completed image roundness, spacing, layout, width, and desktop/mobile
ratio policies.

Core3 migrations 134/135 add a durable company-scoped columns-order policy and
deterministic Regular order fixture. Separate
`pages/product-page-columns-order-policy.yaml` and
`api/product-page-columns-order-policy.yaml` contracts join by
`ecommerce-product-page-columns-order-policy`; the form requires
`ecommerce.write`, validates both source values, and uses row-version
optimistic concurrency. Product Detail has a separate read projection for the
effective company order and displays the current choice. Migration replay and
DuckDB restart preserve the setting; invalid, foreign-company, and stale
writes are rejected.

Focused source/contract, CRUD, permission, validation, projection, replay, and
restart coverage is recorded under
`evidence/ecommerce/2026-09-21/ecom-catalog-product-page-columns-order-001/`.
Authenticated Core3 desktop/mobile capture is blocked by unavailable ports
3000/4312/4313 and no persistent browser runtime. Odoo `/shop` is an exact
HTTP 404 on 8069 and 8073, so paired rendering remains blocked. This bounded
slice is not Ecommerce module sign-off.
...
The supplied `website` model defines `product_page_image_spacing` as None,
Small, Medium, or Big; the product template emits the selected value as
`data-image_spacing` for the product image grid. This is a separate source
setting from the completed image layout, width, and desktop/mobile ratio
policies.

Core3 migrations 130/131 add a durable company-scoped spacing policy and
deterministic None fixture. Separate
`pages/product-page-image-spacing-policy.yaml` and
`api/product-page-image-spacing-policy.yaml` contracts join by
`ecommerce-product-page-image-spacing-policy`; the form requires
`ecommerce.write`, validates the four source values, and uses row-version
optimistic concurrency. Product Detail has a separate read projection for the
effective company spacing and displays the current choice. Migration replay
and DuckDB restart preserve the setting; invalid, foreign-company, and stale
writes are rejected.

Focused source/contract, CRUD, permission, validation, projection, replay, and
restart coverage is recorded under
`evidence/ecommerce/2026-09-21/ecom-catalog-product-page-image-spacing-001/`.
Authenticated Core3 desktop/mobile capture is blocked by unavailable ports
3000/4312/4313 and no persistent browser runtime. Odoo `/shop` is an exact
HTTP 404 on 8069 and 8073, so paired rendering remains blocked. This bounded
slice is not Ecommerce module sign-off.
