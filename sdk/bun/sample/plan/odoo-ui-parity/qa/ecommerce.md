...
| ECOMMERCE-PERM-013 | Authenticated customer ownership cannot be bypassed with caller-supplied customer/cart IDs | Ecommerce Checkout scope test — 4 tests, 22 assertions; non-admin email/scope context returns no other-customer orders and rejects checkout with `ECOMMERCE_CHECKOUT_OWNERSHIP_REQUIRED` | pass at YAML service/mutation level; HTTP actor matrix remains pending |
