...
| ECOMMERCE-PERM-013 | Authenticated customer ownership cannot be bypassed with caller-supplied customer/cart IDs | `bun test ./test/ecommerce_checkout.integration.test.ts` — 5 tests, 26 assertions; HTTP query with `buyer@acme.example` ignores another customer ID and returns only the authenticated customer’s order | pass for HTTP read boundary; authenticated mutation actor and wrong-company matrix remain pending |
