...
| ECOMMERCE-FUNC-009 | Shop page/API, published-product visibility and cart navigation contract | `bun test ./test/ecommerce_shop.integration.test.ts` — 3 tests, 17 assertions; public operation query and route return only active/published catalog rows; authenticated add/repeat-add persists the customer cart line and quantity | pass for authenticated cart mutation; anonymous cart remains planned |
| ECOMMERCE-FUNC-014 | Product catalog import and export affordance | `bun test ./test/ecommerce_products.integration.test.ts` — 2 tests, 21 assertions; `bun run audit` passed with 658 pages, 668 routes, and 1130 datasources; import validates `Name|Internal Reference|Sales Price` rows and persists products; ListView exposes shared export controls | pass for YAML import and shared export contract; authenticated browser interaction remains open |
| ECOMMERCE-PERM-014 | Public catalog boundary | `bun test ./test/ecommerce_shop.integration.test.ts` — public route test verifies GET/search response, rejects POST with 405, and operation SQL matches persisted published fixtures | pass for public catalog read; public cart/customer isolation remains open |
...
| ECOMMERCE-WF-017 | Cart price-list recalculation | Cart test — applies the retail price list to the authenticated owned cart, persists `pricelist_id`, recalculates the chair line from 249 USD to 229 USD, and increments line versions; isolated authenticated Chrome capture at `/tmp/core3-odoo-parity/ecommerce-checkout-20260913/desktop-cart-pricelist.png` selected `Retail Customers` and changed the visible total from 285 to 265 | pass at service/API and desktop browser interaction level; mobile price-list state and paired Odoo comparison remain open |
...
| ECOMMERCE-FUNC-013 | Product image asset upload/download and metadata persistence | `bun test ./test/ecommerce_product_detail.integration.test.ts` — 3 tests, 18 assertions; multipart upload writes the file, inserts metadata, and download returns the stored bytes | pass for image asset path; catalog import/export remains pending |
