...
| ECOMMERCE-PERM-014 | Public catalog and anonymous cart boundary | `bun test ./test/ecommerce_shop.integration.test.ts` — public route verifies GET/search, rejects unsupported methods, validates a cookie-scoped cart ID, and returns the same cart through the public cart operation | pass for public catalog and anonymous-cart isolation; public browser journey remains open |
...
| ECOMMERCE-WF-018 | Anonymous cart persistence | Same Shop test — public POST creates a `ecommerce-cart-anon-*` cookie cart, repeated mutation increments quantity, GET returns the persisted line/total, and a customer-owned cart ID is rejected | pass at service/API contract level; public browser journey and paired Odoo comparison remain open |
