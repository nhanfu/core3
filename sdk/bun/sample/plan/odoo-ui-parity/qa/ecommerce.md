...
| ECOMMERCE-WF-021 | Payment/delivery durable boundary contract | `bun test ./test/ecommerce_checkout.integration.test.ts` — 7 tests, 41 assertions; `services/ecommerce/temporal/payment-delivery.yaml` declares Temporal identity, task queue, permissioned idempotent activities, callbacks, retry policy, timeout, and compensation | contract pass; Temporal worker/start/replay/retry/shutdown execution remains pending because no Temporal SDK/server is installed |
