...
| ECOMMERCE-WF-021 | Payment/delivery durable boundary contract | YAML contract test plus Bun runtime smoke: SDK `1.23.0` worker bundle reached `RUNNING`, workflow returned `Authorized/Ready`, worker restart recovered the original timer-waiting workflow to `COMPLETED`, and shutdown reached `STOPPED` against temporary Temporal Server `1.31.2`; checkout contract suite 7 tests, 41 assertions | pass for Bun startup, workflow execution, timer recovery, and shutdown; provider retry, callback delivery, and compensation failure-path evidence remains open |
