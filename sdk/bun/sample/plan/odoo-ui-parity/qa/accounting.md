...
| ACC-FUNC-009-CN | Invoice Credit Note reversal | `/accounting/invoice-detail` | Posted customer/vendor document creates one linked draft credit note/refund with durable date/reason and stale/type/date guards | `accounting_invoice_reversal.integration.test.ts` | pass at focused API/storage level; browser modal and generated-document posting remain open |
| ACC-INVOICE-CANCEL-001 | Invoice Cancel | `/accounting/invoice-detail` | Draft non-journal invoice exposes Odoo-shaped Cancel and persists guarded Draft → Cancelled transition | `evidence/accounting/2026-09-22/ACC-INVOICE-CANCEL-001/`; `accounting_invoice_cancel.integration.test.ts` | contract/persistence pass; live tab borrow blocked, no visual claim |
...
## QA disposition — invoice Pay Now candidate (2026-09-22)

- **ACC-INVOICE-PAY-NOW-001:** focused integration coverage passes (**3 tests / 18 assertions**) for Odoo `payment_pay` source mapping, page/API `page.id` binding, pending transaction creation, duplicate/stale/invalid guards, and DuckDB restart persistence.
