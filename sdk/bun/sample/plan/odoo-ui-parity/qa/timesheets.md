...
## `TIMESHEET-ALL-TASK-FILTER-001` — All Timesheets Task filter (2026-09-21)

- Source gate: Odoo's All Timesheets search view exposes the structured `task_id` filter.
- Core3 gate: the separate `all-timesheets` page/API pair exposes a manager-scoped open-task filter, projects durable `task_id` into list/pivot data, and applies current-company and empty-fixture guards.
- Focused gate: `test/timesheets_all_task_filter.integration.test.ts` passes 3/3 tests / 21 expectations; the existing All Timesheets regression is rerun with this slice.
...
- Core3 gate: the separate `all-timesheets` page/API pair exposes a manager-scoped active-project filter, projects durable `project_id` into list/pivot data, and applies the current-company and empty-fixture guards.
- Focused gate: `test/timesheets_all_project_filter.integration.test.ts` passes 3/3 tests / 20 expectations; the existing All Timesheets regression is rerun with this slice.
- Odoo browser gate: authenticated desktop filters Research & Development and renders `1-80 / 159`; authenticated mobile renders responsive Kanban. Artifacts are under `evidence/timesheets/2026-09-21/timesheet-all-project-filter/`.
- Core3 browser blocker: the bounded startup probe did not expose backend `3001/api/modules` before timeout, so desktop/mobile authentication evidence is unavailable; exact output is in `core3-readiness.txt`. Odoo Print/PDF/action blockers remain open and this slice is not sign-off.
...
## `TIMESHEET-ALL-BILLED-ON-MILESTONES-FILTER-001` — All Timesheets Billed on Milestones filter (2026-09-21)

- Source gate: `sale_timesheet` inserts `<filter name="billable_milestones" string="Billed on Milestones" domain="[('timesheet_invoice_type', '=', 'billable_milestones')]"/>` into the inherited All Timesheets search view.
- Core3 gate: the paired `all-timesheets` page/API search contract adds a manager-scoped Billed on Milestones option, records the durable `billing_type` filter contract, exposes pivot billing type, and applies current-company and empty-fixture guards.
- Focused gate: `test/timesheets_all_billed_on_milestones_filter.integration.test.ts` passes 4/4 tests / 21 expectations, including relation-update freshness and file-backed restart.
...
| Check | Expected evidence | Result |
| --- | --- | --- |
| Odoo source/action comparison | `action_view_subtask_timesheet`, descendant task scope, `default_project_id: self.project_id.id` | pass in focused source test |
| Paired YAML contract | layout-only `task-timesheets` page and API `page.id` binding | pass |
| Durable context/prefill | current-company task/project relation, source-prefilled create form | pass |
| Permission/company/missing/empty guards | `timesheets.write`, active timesheetable project, current company, empty/missing fixtures | pass |
| Stale/concurrency boundary | project context membership and task/project relation rejected before write | pass |
| Restart | migration replay and file-backed task/project context survive reopen | pass |
| Focused regression | new test 4/4 (23 expectations); related task regression 26/26 (151 expectations) | pass |
| Audit/lint/diff | UI audit 754/763/1523; ESLint; staged `git diff --check` | pass after staging |
| Authenticated desktop/mobile evidence | Core3 and paired Odoo task action capture | blocked; Core3 3001 refused connections and Odoo exposed only `/web/login`; no sign-off |
