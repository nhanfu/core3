...
| SALE_SUBSCRIPTION-PERM-001 | Enforce read/write/manage boundaries, including direct plan and lifecycle mutations | `2e07002b`: focused actor test passed with 33 assertions; all 10 direct mutation actions return 403 to a read-only actor; authenticated runtime checks returned reader lifecycle 403, writer plan 403, manager plan 200, writer lifecycle 200 | pass for Core3 |
...
- `bun test test/sale_subscription_actor_boundary.integration.test.ts`: 1 pass, 33 assertions, 0 failures. The test uses authenticated actor claims against the YAML API, audits all 10 direct mutation actions, and verifies the permission boundary plus lifecycle persistence/invoice creation. This follow-up is `2e07002b`; baseline evidence is `56f07523`, with its documentation pin in `0027f1aa`.
