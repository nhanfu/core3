...
| BLOG-ASSET-001 | Post attachment upload/download | `bun test ./test/blog_post_assets.integration.test.ts` — 1 test, 8 assertions; authenticated multipart upload persists `blog_post_attachments` metadata and permissioned download returns the original 4 bytes | pass for persisted attachment contract; public asset policy, editor rendering and Odoo comparison remain open |
