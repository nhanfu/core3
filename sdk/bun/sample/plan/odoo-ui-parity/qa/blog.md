...
| BLOG-PUBLIC-002 | Public post renderer | `bun test ./test/blog_public.integration.test.ts` — renderer contract asserts Fluent HTML, module-owned published post API, and `/blog/post` route; authenticated-free browser checks at 1440x900 and 390x844 rendered published post with no errors/overflow, while draft returned Post unavailable | pass for Core3 public runtime; paired Odoo comparison and richer content/assets remain open |
| BLOG-ASSET-001 | Post attachment upload/download | `bun test ./test/blog_post_assets.integration.test.ts` — 2 tests, 13 assertions; authenticated multipart upload persists `blog_post_attachments` metadata, exact bytes remain downloadable after DuckDB close/reopen, and the upload root is cleaned up | pass for persisted/restart attachment contract; public asset policy, editor rendering and Odoo comparison remain open |
