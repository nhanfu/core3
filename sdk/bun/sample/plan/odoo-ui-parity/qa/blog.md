...
| BLOG-FUNC-003 | Post import/export/print | `bun test ./test/blog_blogs.integration.test.ts` — validated `Title|Subtitle|Author|Tags` import, invalid-row rejection, idempotent upsert, and the detail Print action; authenticated mobile browser exposed Import, imported a post, and exposed Print | pass for Core3 contract/runtime; full Odoo export comparison remains open |
