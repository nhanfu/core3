...
- ECOM-FUNC-073: trace Website Sale
  `website.shop_opt_products_design_classes`, the Products Design Panel Add
  to Cart control, `o_wsale_products_opt_has_cta`, the product-tile quick-add
  visibility boundary, and `/shop/config/website`; verify separate Core3 Shop
  policy and page/API contracts.
- ECOM-WF-084: replay the deterministic visible-CTA policy, hide and restore
  the Shop Add to Cart control, project the effective Shop setting, reject
  invalid, foreign-company, missing, and stale writes, and preserve it across
  migration replay and DuckDB restart.
- ECOM-PERM-080: require `ecommerce.read` for policy/Shop reads and
  `ecommerce.write` for updates; enforce company scope and optimistic
  row-version guards.
- ECOM-UI-066: render the configuration form and Shop projection at desktop
  and mobile and compare the authenticated Odoo Products Design Panel Add to
  Cart control; Core3 runtime is unavailable and Odoo `/shop` is HTTP 404 in
...
- ECOM-FUNC-069: trace Website Sale `shop_gap`, its `setGap` builder action,
  0–28px range, CSS custom property, and persistence boundary; verify
  separate Core3 Shop grid-gap policy page/API contracts.
- ECOM-WF-080: replay the deterministic 16px policy, change supported gaps,
...
- ECOM-PERM-076: require `ecommerce.read` for policy/options/Shop reads and
  `ecommerce.write` for updates; enforce company scope and optimistic
  row-version guards with safe missing-record and fixture replay behavior.
- ECOM-UI-062: render the grid-gap form and Shop projection at desktop and
  mobile and compare the authenticated Odoo Products Design Panel behavior;
...
- ECOM-FUNC-066: trace Website Sale `shop_page_container`, its Regular and
  Full-width values, and the `website.shop_page_container == 'fluid'` shop
  template boundary; verify separate Core3 policy and Shop page/API contracts.
- ECOM-WF-077: replay the deterministic Regular policy, switch to Full-width,
  project the effective Shop container, reject invalid/foreign/stale writes,
  and preserve the policy across migration replay and DuckDB restart.
- ECOM-PERM-073: require `ecommerce.read` for policy/options/Shop reads and
  `ecommerce.write` for updates; enforce company scope and optimistic
  row-version guards with idempotent fixture/update behavior.
- ECOM-UI-059: render the configuration form and Shop projection at desktop
  and mobile and compare the authenticated Odoo Shop container behavior;
  Core3 runtime is unavailable and Odoo `/shop` is HTTP 404 in the supplied
  references.
