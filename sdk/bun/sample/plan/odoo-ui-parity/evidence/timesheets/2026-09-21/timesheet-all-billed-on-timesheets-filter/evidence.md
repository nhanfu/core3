# `TIMESHEET-ALL-BILLED-ON-TIMESHEETS-FILTER-001`

## Source and Core3 contract

Odoo's `sale_timesheet` search inheritance adds the authenticated All
Timesheets `billable_time` filter labelled **Billed on Timesheets**, over
`timesheet_invoice_type = billable_time`. Core3 maps that action through the
existing durable `timesheet_entries.billing_type` relation. The page declares
the one filter and the API exposes the durable billing type in its pivot
contract and applies the same company-scoped predicate under
...
- `odoo-desktop.png`: 1440x900 authenticated All Timesheets search panel with
  Billed on Timesheets selected; the result list renders `1-80 / 92`.
