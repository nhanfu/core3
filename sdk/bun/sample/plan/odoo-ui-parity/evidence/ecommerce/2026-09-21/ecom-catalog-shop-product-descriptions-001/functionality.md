...
- Deterministic My Company fixture starts with product-card descriptions
  visible, matching Odoo's default class list.
- Policy reads use `ecommerce.read`; the update uses `ecommerce.write` and
  requires optimistic row-version concurrency.
- The checkbox can be changed to hide descriptions, the effective setting is
