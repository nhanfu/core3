...
Disposition: PASS for the bounded Core3 lifecycle, persistence, permissions,
and authenticated evidence. PARTIAL for repository-wide verification because
the shared `discoverPages()` boundary is blocked by unrelated module edits.
Residual Odoo/Core3 gaps are explicitly listed in `source-comparison.md`.
