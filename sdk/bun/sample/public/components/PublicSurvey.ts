...
function progressionLabel(index: number, total: number, mode: string): string {
  const answered = Math.max(0, Math.min(index, total));
  return mode === 'percent'
    ? `${Math.round((answered / Math.max(total, 1)) * 100)}% completed`
    : `${answered} of ${total} answered`;
}

function renderQuestion(container: HTMLElement, question: SurveyQuestion, index: number, total: number, answer: string | string[] | Record<string, string[]> = '', comment = '', surveyToken = '', answerToken = '', usersCanGoBack = false, progressionMode = 'percent') {
  ...
  container.innerHTML = `
    ...
    ${input}
    ${commentInput}
    <div class="core3-public-survey__footer"><span class="core3-public-survey__progress">${escapeHtml(progressionLabel(index, total, progressionMode))}</span><div class="core3-public-survey__footer-actions">${index > 0 && usersCanGoBack ? '<button class="core3-public-survey__button" data-back type="button">Back</button>' : ''}<button class="core3-public-survey__button" data-next type="button">${index === total - 1 ? 'Submit' : 'Next'}</button></div></div>
  `;
}
...
export async function mount(outlet: HTMLElement, token: string, initialAnswerToken = '') {
  ...
  function renderCurrentQuestion() {
    ...
    renderQuestion(body, currentQuestion, questionIndex, questions.length, answers[currentQuestion.id], String(answers[`${currentQuestion.id}__comment`] || ''), token, answerToken, survey.users_can_go_back !== false, String(survey.progression_mode || 'percent'));
    startTimer();
