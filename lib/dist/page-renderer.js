...
export async function renderPage(config, { container = document.body } = {}) {
    ...
    for (const src of sourceDefs.values()) {
        ...
        filterState[src.id] = {};
        try {
            const result = await client.query(createQuery({ sourceId: src.id, params: {}, skip: 0, top: pageSize }));
    ...
    async function refetchSource(sourceId, filters = {}, skip = 0, top = 25) {
        const result = await client.query(createQuery({ sourceId, params: filters, skip, top }));
    ...
    async function handleAction(actionDef, row) {
        ...
        switch (actionDef.type) {
            case 'form':
                ...
                break;
            case 'delete': {
                const msg = interpolate(actionDef.confirm || 'Delete this record?', { ...ctx, row: row || {} });
                if (!confirm(msg))
                    return;
                await client.patch({
                ...
                if (actionDef.refresh?.length)
                    await refreshSources(actionDef.refresh);
                break;
            }
            case 'patch': {
            ...
            case 'navigate':
                navigate(actionDef.navigate_to, actionDef.params
    ...
    async function renderStatRow(def, targetContainer) {
        ...
        comp.mount(slot);
        if (def.source) {
            if (!boundComponents[def.source])
                boundComponents[def.source] = [];
    ...
    async function renderGridView(def, targetContainer) {
        ...
        const comp = new GridView(`grid-${sourceId || Date.now()}`, { rows: sourceResult.data || [], meta: sourceResult.meta }, columnDefs);
        ...
        comp._cellState = (colDef, row) => {
            ...
            switch (colDef.type) {
                ...
                case 'ActionCell': {
                    ...
                    const visibleActions = (colDef.actions || []).filter(a => !a.show_if || evalExpr(a.show_if, rowCtx));
        ...
        comp.mount(slot);
        if (sourceId) {
            if (!boundComponents[sourceId])
                boundComponents[sourceId] = [];
    ...
    async function renderTabGroupDef(def, targetContainer) {
        ...
        const visibleTabs = (def.tabs || []).filter(tab => !tab.permission || userPerms.includes(tab.permission));
    ...
    if (config.toolbar?.length) {
        ...
        for (const btn of config.toolbar) {
            if (btn.permission) {
                const userPerms = ctx.user.permissions || [];
                if (!userPerms.includes(btn.permission))
                    continue;
            }
            ...
            button.addEventListener('click', async () => {
                ...
                if (actionDef)
                    await handleAction(actionDef, null);
                else
                    console.warn(`[page-renderer] Toolbar action not found: ${btn.action}`);
            });
    ...
    if (config.filters) {
        ...
        const filterBar = new FilterBar('page-filter-bar', { values: {} }, config.filters.fields || []);
        filterBar._onAction = async (actionId, params) => {
            ...
            try {
                const data = await refetchSource(filterSourceId, values, 0, paginationState[filterSourceId].top);
